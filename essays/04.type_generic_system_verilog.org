...
泛型类或者说类模板对于C++的一个重要意义是使之可以提供各种泛型的数据结构——容器。然而，诸如vector，map之类的容器，在SystemVerilog中已经有对应的内置支持，即数组（array）和关联数组（associative array）。所以，泛型类之于SystemVerilog只能说是有所裨益。另一方面，SystemVerilog缺乏对另一种重要的泛型形式——泛型函数——的支持。好在，还有补救方法。
