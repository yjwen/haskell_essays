...
SystemVerilog不支持像C++函数模板那样地直接定义泛型的函数(function或task)。要想实现一个泛型的函数，只能利用泛型类的静态成员函数。例如，要实现一个泛型的矩阵转置函数，可以采用如下的代码。
#+BEGIN_SRC verilog
...
#+BEGIN_SRC verilog
  ...
  om = RealM4x8::transpose(im); // 假定im是个4x8的实数矩阵，om是一个8x4的实数矩阵
#+END_SRC
