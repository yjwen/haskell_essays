...
#+BEGIN_SRC verilog
  ...
  endclass // Pair
#+END_SRC
简单解释一下这段代码。在井号（#）之后的是 ~Pair~ 的参数列表。其中，类型参数以关键字 ~type~ 开头，后接类型参数名。与C++的模板参数不同，SystemVerilog的参数必须有一个默认値。此处，类型参数 ~T0~ 和 ~T1~ 的默认値都设为 ~int~ 。类型参数可用于定义成员变量的类型，成员函数的参数类型，返回类型等等。通过为类型参数 ~T0~ 和 ~T1~ 指定不同的类型，我们可以得到保存不同类型的数据的二元组类。下面的代码列出了几种指定类型参数的方式。
#+BEGIN_SRC verilog
...
（套用一个卖弄专用的表达方法）熟悉C++泛型编程的朋友应该已经想到，有了泛型类的支持，我们就可以在SystemVerilog中构造泛型的容器了。很遗憾，诸如vector，map之类的C++容器，在SystemVerilog中已经有对应的内置支持，即数组（array）和关联数组（associative array）。利用泛型类构造的容器，已成鸡肋。虽如此，泛型类与SystemVerilog中的另一种（平平无奇）的语法相结合后，仍然给能我们相当的想像空间
